...
namespace Tests.Controllers.LockControllerTests
{
    ...
    public class UnlockTest
    {
        ...
        public async Task UnLockSuccess()
        {
            ...
            var date = new DateOnly(2025, 1, 1);
            var districtId = 1;
            var lockIt = new LockModel { Date = date, DistrictId = districtId };
            _mockLockRepository.Setup(r => r.Get(It.IsAny<Expression<Func<LockModel, bool>>>(), "")).ReturnsAsync(new List<LockModel> { lockIt });
            ...
            _mockLockRepository.Verify(r => r.Delete(It.IsAny<LockModel>()), Times.Once);
